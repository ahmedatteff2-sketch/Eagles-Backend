...
const navItems = [
  ...
  {
    path: "/admin/absent-members",
    label: "الأعضاء الغائبون",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
      ...
      >
        ...
        >
          ...
          <kbd
            ...
            style={{ background: "hsl(0 0% 14%)", color: "hsl(0 0% 45%)" }}
          >
            ESC
          </kbd>
        </div>
        {q.length === 0 && (
          <div className="px-4 py-8 text-center">
            <p className="text-muted-foreground text-sm">اكتب اسم العضو أو رقم الهاتف</p>
          </div>
        )}
        ...
        {results.length > 0 && (
          <div className="py-2">
            {results.map((u: any) => {
              const isActive = u.currentSubscription?.endDate
                ? new Date(u.currentSubscription.endDate) >= new Date()
                : false;
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {count > 0 && (
          <span
            className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 rounded-full flex items-center justify-center text-white"
            style={{
      ...
      {open && (
        <div
          className="absolute left-0 top-full mt-2 w-72 rounded-xl shadow-2xl z-50 overflow-hidden"
          ...
          >
            ...
            {count > 0 && (
              <span
                ...
                style={{ background: "hsl(0 72% 50% / 0.15)", color: "hsl(0 72% 60%)" }}
              >
                {count}
              </span>
            )}
            ...
            <div className="max-h-64 overflow-y-auto">
              {expiringToday.length > 0 && (
                <div className="px-3 py-2">
                  <p className="text-xs font-bold mb-1.5" style={{ color: "hsl(0 72% 60%)" }}>
                    ⚠ تنتهي اليوم
                  </p>
              ...
              {expiringSoon.filter((m: any) => !expiringToday.includes(m)).length > 0 && (
                <div
                  className="px-3 py-2"
                  style={{ borderTop: expiringToday.length > 0 ? "1px solid hsl(0 0% 12%)" : "none" }}
                >
                  <p className="text-xs font-bold mb-1.5" style={{ color: "hsl(40 65% 52%)" }}>
                    ⏰ تنتهي قريباً
                  </p>
                  {expiringSoon
                    .filter((m: any) => !expiringToday.includes(m))
                    .map((m: any) => {
                      const d = Math.ceil(
                        (new Date(m.currentSubscription.endDate).getTime() - now.getTime()) / 86400000,
                      );
                      return (
                        <Link key={m.id} href={`/admin/members/${m.id}`}>
                          <div
                            onClick={() => setOpen(false)}
                            className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-lg cursor-pointer hover:bg-white/5 transition-colors"
                          >
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const todayCount: number = Array.isArray(todayCheckins)
    ? todayCheckins.length
    : ((todayCheckins as any)?.data?.length ?? (todayCheckins as any)?.total ?? 0);
    ...
    return (
      <Link href={item.path}>
        ...
        >
          ...
          {collapsed && showBadge && (
            <span
              className="absolute -top-0.5 -left-0.5 w-4 h-4 rounded-full text-center text-white flex items-center justify-center"
          ...
          {collapsed && (
            <div
              className="absolute left-full ml-3 px-2.5 py-1.5 rounded-lg text-xs font-semibold pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50"
              style={{
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "hsl(0 0% 5%)" }} dir="rtl">
      ...
      {/* ═══════════ Desktop Sidebar ═══════════ */}
      <aside
        className={`hidden md:flex ${w} flex-shrink-0 flex-col transition-all duration-300`}
        style={{ background: "hsl(0 0% 3%)", borderLeft: "1px solid hsl(0 0% 10%)" }}
      >
        {/* Header */}
        <div
          className="flex items-center gap-3 px-4 py-4 relative"
          style={{ borderBottom: "1px solid hsl(0 0% 10%)", minHeight: 72 }}
        >
          <div
          ...
          <Link href="/admin">
            <div className="flex items-center gap-3 cursor-pointer">
              ...
              {!collapsed && (
                <div>
                  <div
                    className="font-black text-sm leading-tight tracking-widest uppercase"
                    style={{ color: "hsl(40 65% 55%)" }}
                  >
                    Eagle Gym
                  </div>
                  <div className="text-xs" style={{ color: "hsl(0 0% 35%)" }}>
                    إدارة النادي
                  </div>
                </div>
              )}
          ...
          {collapsed && (
            <button
              ...
              className="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center z-10 shadow-lg"
        ...
        <div className="px-3 py-2.5" style={{ borderBottom: "1px solid hsl(0 0% 8%)" }}>
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.borderColor = "hsl(40 65% 48% / 0.3)";
            ...
            {!collapsed && (
              <>
                ...
                <kbd
                  ...
                  style={{ background: "hsl(0 0% 14%)", color: "hsl(0 0% 35%)" }}
                >
                  ⌘K
                </kbd>
              </>
        ...
        <div className="p-3" style={{ borderTop: "1px solid hsl(0 0% 10%)" }}>
          {!collapsed && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold truncate" style={{ color: "hsl(40 20% 80%)" }}>
                  {user?.name}
                </p>
                <p className="text-xs" style={{ color: "hsl(40 65% 45%)" }}>
                  مسؤول
                </p>
              </div>
              <button
          ...
          <button
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.background = "transparent";
              e.currentTarget.style.color = "hsl(0 0% 38%)";
            }}
          >
            <svg
              viewBox="0 0 24 24"
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        {/* Mobile top bar */}
        <div
          className="md:hidden flex items-center justify-between px-3 py-2.5 flex-shrink-0"
          ...
          <div className="md:hidden fixed inset-0 z-50 flex" dir="rtl">
            ...
            >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate" style={{ color: "hsl(40 20% 80%)" }}>
                    {user?.name}
                  </p>
                  <p className="text-xs" style={{ color: "hsl(40 65% 45%)" }}>
                    مسؤول
              ...
              <div className="p-3 space-y-2" style={{ borderTop: "1px solid hsl(0 0% 10%)" }}>
                <div className="flex gap-2">
                  <button
                    onClick={toggleTheme}
                    className="flex-1 text-xs py-2.5 px-3 rounded-xl flex items-center justify-center gap-2"
                  ...
                  <Link href="/admin/settings">
                    <button
                      ...
                      className="text-xs py-2.5 px-4 rounded-xl flex items-center gap-2"
                      style={{ background: "hsl(0 0% 7%)", color: "hsl(0 0% 55%)" }}
                    >
                      ⚙️ إعدادات
                    </button>
                ...
                <button
                  onClick={handleLogout}
                  className="w-full text-xs py-2.5 px-3 rounded-xl flex items-center justify-center gap-2"
                  style={{
        ...
        <div
          className="md:hidden fixed bottom-0 inset-x-0 z-40 flex items-center justify-around px-1 py-1"
          style={{
          ...
          {mobileBottomTabs.map((tab) => {
            ...
            return (
              <Link key={tab.path} href={tab.path}>
                <div
                  className="flex flex-col items-center gap-0.5 py-1.5 px-3 rounded-xl transition-colors min-w-[52px]"
