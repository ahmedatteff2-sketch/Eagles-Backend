      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight flex items-center gap-3" style={{ color: "hsl(0 0% 90%)" }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "rgba(37,211,102,0.12)", border: "1px solid rgba(37,211,102,0.3)" }}>
        ...
        <button onClick={openCreate}
          ...
          style={{ background: "linear-gradient(135deg, hsl(40 65% 52%), hsl(40 65% 40%))", color: "hsl(0 0% 5%)", boxShadow: "0 4px 20px hsl(40 65% 48% / 0.3)" }}>
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-xl p-5 animate-pulse" style={{ background: "hsl(0 0% 7%)", border: "1px solid hsl(0 0% 12%)" }}>
        ...
        <div className="rounded-2xl p-12 text-center" style={{ background: "hsl(0 0% 7%)", border: "1px dashed hsl(0 0% 15%)" }}>
          ...
          <h3 className="text-lg font-bold mb-2" style={{ color: "hsl(0 0% 70%)" }}>لا توجد قوالب بعد</h3>
          <p className="text-sm mb-6" style={{ color: "hsl(0 0% 40%)" }}>أنشئ أول قالب واستخدم متغيّرات زي {"{name}"} و {"{end_date}"} عشان الرسائل تتخصّص لكل عضو تلقائياً.</p>
          <button onClick={openCreate} className="px-5 py-2 rounded-xl text-sm font-bold" style={{ background: "hsl(40 65% 48% / 0.15)", color: GOLD, border: "1px solid hsl(40 65% 48% / 0.3)" }}>
            إضافة أول قالب
          </button>
        </div>
      ) : (
              ...
              <div className="flex items-start gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <h3 className="text-base font-bold" style={{ color: "hsl(0 0% 88%)" }}>{t.name}</h3>
                  ...
                  <pre className="text-sm whitespace-pre-wrap leading-relaxed" style={{ fontFamily: "inherit", color: "hsl(0 0% 65%)" }}>{t.body}</pre>
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button onClick={() => toggleEnabled(t)} className="relative" style={{ width: 42, height: 24, borderRadius: 999, background: t.enabled ? "hsl(142 60% 45%)" : "hsl(0 0% 20%)" }}>
                    <div className="absolute top-0.5 rounded-full bg-white transition-all" style={{ width: 20, height: 20, left: t.enabled ? 20 : 2 }} />
                  </button>
                  <button onClick={() => openEdit(t)} className="p-2 rounded-lg" style={{ color: "hsl(0 0% 50%)" }} title="تعديل">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
                      <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                    </svg>
                  </button>
                  <button onClick={() => setConfirmDelete(t)} className="p-2 rounded-lg" style={{ color: "hsl(0 60% 55%)" }} title="حذف">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
          ...
          <div className="w-full max-w-2xl rounded-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
            ...
            <div className="p-5 sm:p-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={closeForm} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "hsl(0 0% 14%)" }}>
              ...
              <div>
                <label className="block text-xs font-semibold mb-1.5" style={{ color: "hsl(0 0% 55%)" }}>اسم القالب</label>
                <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                  className="w-full rounded-lg px-3 py-2.5 text-sm focus:outline-none" placeholder="مثال: تذكير قبل انتهاء الاشتراك"
                  style={{ background: "hsl(0 0% 12%)", border: "1px solid hsl(0 0% 22%)", color: "hsl(0 0% 90%)" }} />
              </div>

              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-semibold" style={{ color: "hsl(0 0% 55%)" }}>نص الرسالة</label>
                  <span className="text-xs" style={{ color: "hsl(0 0% 40%)" }}>{form.body.length} / 2000</span>
                </div>
                <textarea id="wa-tpl-body" value={form.body} onChange={(e) => setForm((f) => ({ ...f, body: e.target.value }))}
                  rows={6}
              ...
              <div>
                <label className="block text-xs font-semibold mb-2" style={{ color: "hsl(0 0% 55%)" }}>المتغيّرات المتاحة (اضغط للإدراج)</label>
                <div className="flex flex-wrap gap-2">
                  {SUPPORTED_VARS.map((v) => (
                    <button key={v.key} type="button" onClick={() => insertVarAtCursor(v.key)}
                      className="px-2.5 py-1 rounded-lg text-xs font-mono transition-colors"
                      style={{ background: "hsl(40 65% 48% / 0.12)", color: "hsl(40 65% 60%)", border: "1px solid hsl(40 65% 48% / 0.25)" }}
                      title={v.label}>
                      {`{${v.key}}`}
                    </button>
                  ))}
              ...
              {form.body.trim() && (
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: "hsl(0 0% 55%)" }}>معاينة (بقيم تجريبية)</label>
                  <pre className="rounded-lg px-3 py-2.5 text-sm whitespace-pre-wrap leading-relaxed"
                    style={{ background: "rgba(37,211,102,0.06)", border: "1px solid rgba(37,211,102,0.2)", fontFamily: "inherit", color: "hsl(0 0% 80%)" }}>
                    {previewBody}
                  </pre>
                </div>
              )}
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={closeForm} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ background: "hsl(0 0% 14%)", color: "hsl(0 0% 70%)" }}>
                  إلغاء
                </button>
                <button onClick={save} disabled={saving} className="flex-1 py-2.5 rounded-xl text-sm font-bold"
                  style={{ background: "linear-gradient(135deg, hsl(40 65% 52%), hsl(40 65% 42%))", color: "hsl(0 0% 5%)", opacity: saving ? 0.6 : 1 }}>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)" }}>
          <div className="w-full max-w-sm rounded-2xl overflow-hidden p-5" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 16%)" }}>
            ...
            <p className="text-center text-sm mb-5" style={{ color: "hsl(0 0% 55%)" }}>سيتم حذف "{confirmDelete.name}" نهائياً.</p>
            <div className="flex gap-2">
              <button onClick={() => setConfirmDelete(null)} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ background: "hsl(0 0% 14%)", color: "hsl(0 0% 70%)" }}>إلغاء</button>
              <button onClick={doDelete} className="flex-1 py-2.5 rounded-xl text-sm font-bold" style={{ background: "hsl(0 72% 51%)", color: "#fff" }}>حذف نهائي</button>
            </div>
          </div>
        </div>
      )}
