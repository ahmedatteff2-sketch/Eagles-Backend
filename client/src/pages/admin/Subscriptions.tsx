            ...
            <div key={sub.id} className="bg-card border border-card-border rounded-xl p-5">
              ...
              <div className="flex gap-2 mt-4">
                <button
                  ...
                  className="flex-1 bg-muted hover:bg-muted/80 text-foreground py-1.5 rounded-lg text-xs font-medium transition-colors"
                >
                  تعديل
                </button>
                <button
                  onClick={() => handleDelete(sub.id)}
                  className="flex-1 bg-destructive/10 hover:bg-destructive/20 text-destructive py-1.5 rounded-lg text-xs font-medium transition-colors"
                >
                  حذف
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-card-border rounded-xl p-6 w-full max-w-sm shadow-xl">
            <h2 className="text-lg font-bold text-foreground mb-4">
              {editing ? "تعديل الخطة" : "إضافة خطة جديدة"}
            </h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
              ...
              <div>
                ...
                />
                {errors.duration && (
                  <p className="text-destructive text-xs mt-1">{errors.duration.message}</p>
                )}
              </div>
              <div>
