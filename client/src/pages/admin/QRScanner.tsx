...
export default function AdminQRScanner() {
  ...
  const processQR = useCallback(
    (raw: string) => {
      ...
      if (trimmed.length > 0) {
        ...
        if (user) {
          doCheckin(String(user.id), user.name);
          return;
        }
      }
    ...
    try {
      ...
      const msg =
        err.name === "NotAllowedError"
          ? "رُفض إذن الكاميرا — اسمح للمتصفح من الإعدادات ثم أعد المحاولة"
          : err.name === "NotFoundError"
            ? "لا توجد كاميرا على هذا الجهاز"
            : err.name === "NotReadableError"
              ? "الكاميرا مستخدمة من تطبيق آخر — أغلقه وأعد المحاولة"
  ...
  const handleFileInput = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const jsQR = jsQRRef.current;
      if (!jsQR) {
        toast({ title: "المكتبة لم تُحمَّل بعد، انتظر ثانية", variant: "destructive" });
        return;
      }
      try {
        const img = new Image();
        ...
        toast({ title: "فشل قراءة الصورة", variant: "destructive" });
      }
      e.target.value = "";
    },
    [processQR, toast],
  ...
  const handleManualCheckin = async () => {
    ...
    const user = users.find(
      (u: any) =>
        String(u.id) === manualId.trim() ||
        u.membershipNumber?.toLowerCase() === searchVal ||
        u.phone === searchVal,
    );
    if (!user) {
      ...
      {lastCheckin && (
        ...
        >
          ...
          <button
            ...
            className="text-green-400/40 hover:text-green-400 text-lg"
          >
            ✕
          </button>
        </div>
                  ...
                  <div className="relative w-56 h-56">
                    {[
                      ...
                      <div
                        ...
                        style={{ borderColor: "hsl(40 65% 52%)" }}
                      />
                    ))}
                    {/* Scanning line animation */}
                    <div
                      className="absolute inset-x-2 h-0.5 animate-bounce"
                      style={{ background: "hsl(40 65% 52% / 0.8)", top: "50%" }}
                    />
                  </div>
              ...
              {cameraError ? (
                ...
                <>
                  <div
                    ...
                    style={{
                      background: "hsl(40 65% 48% / 0.1)",
                      border: "2px dashed hsl(40 65% 48% / 0.4)",
                    }}
                  >
                    <svg
                      viewBox="0 0 24 24"
                  ...
                  <p className="text-muted-foreground text-sm text-center px-6">
                    اضغط الزر لتشغيل الكاميرا — يعمل على Chrome و Firefox و Safari
                  </p>
                </>
              )}
            </div>
      ...
      >
        ...
        <div>
          ...
          >
            ...
            {users.map((u: any) => (
              <option key={u.id} value={u.memberCode || u.id}>
                {u.name} — {u.phone} {u.memberCode ? `(${u.memberCode})` : ""}
              </option>
            ))}
          </select>
        </div>
        <div>
          <label className="block text-xs text-muted-foreground mb-1.5">
            أو أدخل الكود التعريفي، رقم الهاتف، أو ID
          </label>
          <input
            type="text"
            value={manualId}
