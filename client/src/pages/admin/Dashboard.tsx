...
function StatCard({ label, value, sub, icon, color = GOLD, href }: any) {
  ...
  const inner = (
    <div
      className="rounded-xl p-5 flex items-center gap-4 transition-all cursor-pointer group"
      style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 15%)" }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.borderColor = "hsl(40 65% 48% / 0.3)";
        (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)";
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.borderColor = "hsl(0 0% 15%)";
  ...
  useEffect(() => {
    ...
    customFetch<any>("/api/analytics/dashboard")
      .then((d) => {
        ...
        setTopAttendees(d?.topAttendees ?? []);
      })
      .catch(() => {});
  }, []);
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid hsl(0 0% 15%)" }}>
          ...
          >
            <span>🏆</span>
            <h2 className="text-sm font-semibold text-foreground">الأكثر نشاطاً هذا الأسبوع</h2>
          </div>
          <div style={{ background: "hsl(0 0% 8%)" }}>
            {topAttendees.length === 0 ? (
              ...
              <div className="divide-y" style={{ borderColor: "hsl(0 0% 12%)" }}>
                {topAttendees.slice(0, 5).map((m: any, i: number) => (
                  <div key={m.id ?? i} className="flex items-center justify-between px-5 py-2.5">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                        style={{
                          background: i === 0 ? `${GOLD}30` : "hsl(0 0% 14%)",
                          color: i === 0 ? GOLD : "hsl(0 0% 60%)",
                        }}
                      >
                        {i + 1}
                      </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid hsl(0 0% 15%)" }}>
          ...
          >
            <span>😴</span>
            <h2 className="text-sm font-semibold text-foreground">أعضاء غير نشطين (7+ أيام)</h2>
          </div>
          <div style={{ background: "hsl(0 0% 8%)" }}>
            {inactiveMembers.length === 0 ? (
              <div className="px-5 py-8 text-center">
                <p className="text-xs text-muted-foreground">كل الأعضاء نشطين 💪</p>
              </div>
            ) : (
              <div className="divide-y" style={{ borderColor: "hsl(0 0% 12%)" }}>
                {inactiveMembers.slice(0, 5).map((m: any) => (
                  <div key={m.id} className="flex items-center justify-between px-5 py-2.5">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                        style={{ background: "hsl(0 60% 50% / 0.15)", color: "hsl(0 60% 60%)" }}
                      >
                        {m.name?.[0]}
                      </div>
                      <div>
                        <p className="text-sm text-foreground">{m.name}</p>
                        <p className="text-xs text-muted-foreground">{m.daysSince} يوم بدون حضور</p>
                      </div>
                    </div>
                    <a
                      ...
                      style={{ background: "hsl(142 60% 42% / 0.15)", color: "hsl(142 60% 55%)" }}
                    >
            ...
            <div className="divide-y" style={{ borderColor: "hsl(0 0% 12%)" }}>
              ...
              {soonExpiring.slice(0, 6).map((m: any) => {
                const daysLeft = Math.ceil(
                  (new Date(m.currentSubscription.endDate).getTime() - Date.now()) / 86400000,
                );
                return (
                  <div key={m.id} className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-3">
