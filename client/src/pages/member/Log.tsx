      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p
              className={`font-semibold text-sm ${isComplete ? "text-muted-foreground line-through" : "text-foreground"}`}
            >
              {ex.exerciseName}
            </p>
          </div>
          <div className="flex items-center gap-2 mb-2">
            <p className="text-xs text-muted-foreground">
              {total} مجموعات × {ex.reps} تكرار
            </p>
            <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: `${color}15`, color }}>
              {ex.targetMuscle}
            </span>
            {lastWeight > 0 && (
              <span className="text-xs text-muted-foreground">· آخر وزن: {lastWeight} كجم</span>
            )}
  ...
  return (
    <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50" onClick={onClose}>
      <div className="text-center px-6" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-lg font-bold mb-2" style={{ color: "hsl(40 65% 52%)" }}>
          {exerciseName}
        </p>
        <div className="flex items-center justify-center gap-4 mb-4">
          ...
          <div className="text-center">
            <p className="text-xs" style={{ color: "hsl(40 65% 52%)" }}>
              الجديد
            </p>
            <p className="text-3xl font-black" style={{ color: "hsl(40 65% 52%)" }}>
              {weight} كجم
            </p>
          </div>
        </div>
        ...
        <button
          onClick={onClose}
          className="mt-4 px-6 py-2 rounded-xl text-sm font-bold bg-primary text-primary-foreground"
        >
          تمام 💪
        </button>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={onDone}>
      <div className="text-center" onClick={(e) => e.stopPropagation()}>
        ...
        <button
          onClick={onDone}
          className="px-6 py-2 rounded-xl text-sm font-bold bg-primary text-primary-foreground"
        >
          تخطي
        </button>
      </div>
  ...
  return (
    <div className="p-4 space-y-4 pb-8">
      ...
      <div
        ...
        style={{
          background: sessionStart ? "hsl(40 65% 48% / 0.08)" : "hsl(0 0% 9%)",
          border: `1px solid ${sessionStart ? "hsl(40 65% 48% / 0.2)" : "hsl(0 0% 14%)"}`,
        }}
      >
        <div className="flex-1">
          <p className="text-xs text-muted-foreground">
            {sessionStart ? "الجلسة جارية" : "ابدأ جلسة التمرين"}
          </p>
          {sessionStart && (
        ...
        {!sessionStart ? (
          ...
          <button
            onClick={endSession}
            className="px-4 py-2 rounded-xl text-xs font-bold bg-red-500/15 text-red-400 hover:bg-red-500/20 transition-colors"
          >
            ■ إنهاء
          </button>
        )}
      ...
      {activeTemplate?.notes && (
        <div
          ...
          style={{ background: "hsl(40 65% 48% / 0.08)", border: "1px solid hsl(40 65% 48% / 0.15)" }}
        >
              ...
              <div key={ex.id} className="bg-card border border-card-border rounded-xl overflow-hidden">
                ...
                {ex.notes && (
                  <div
                    ...
                    style={{ background: "hsl(0 0% 7%)", borderBottom: "1px solid hsl(0 0% 13%)" }}
                  >
                  ...
                  {weeks.map((week, wi) => {
                    ...
                    return (
                      ...
                      >
                        {/* Week label */}
                        <p
                          className={`text-xs font-bold mb-1.5 ${isCurrent ? "text-primary" : "text-muted-foreground"}`}
                        >
                          {week.label}
                        </p>
                        {/* Weight display */}
                        {bestW !== null ? (
                          <>
                            <p
                              className={`text-lg font-black tabular-nums ${isDone ? "text-green-400" : "text-foreground"}`}
                            >
                              {bestW}
                            </p>
                            ...
                            <div className="flex justify-center gap-0.5 mt-1.5">
                              {Array.from({ length: ex.sets }).map((_, si) => (
                                <div
                                  key={si}
                                  className={`w-2 h-2 rounded-full ${si < setsCount ? "bg-green-400" : "bg-muted"}`}
                                />
                              ))}
                            </div>
                          ...
                          <div className="py-2">
                            <div
                              ...
                              style={{
                                border: isCurrent
                                  ? "2px dashed hsl(40 65% 48% / 0.5)"
                                  : "2px dashed hsl(0 0% 20%)",
                              }}
                            >
                              <svg
                                viewBox="0 0 24 24"
                                fill="none"
                                stroke={isCurrent ? "hsl(40 65% 48%)" : "hsl(0 0% 25%)"}
                                strokeWidth={2}
              ...
              <div className="divide-y" style={{ borderColor: "hsl(0 0% 13%)" }}>
                {progressData.map((d) => (
                  <div key={d.name} className="px-4 py-3 flex items-center justify-between">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex gap-2 mt-1">
                        {d.weekWeights.map((w, i) => (
                          <span
                            key={i}
                            className={`text-xs tabular-nums ${w !== null ? "text-foreground font-bold" : "text-muted-foreground"}`}
                          >
                            {w !== null ? `${w}` : "—"}
                          </span>
                        ))}
                    ...
                    {d.progression !== 0 && (
                      <span
                        className={`text-xs font-bold flex-shrink-0 ${d.progression > 0 ? "text-green-400" : "text-red-400"}`}
                      >
                        {d.progression > 0 ? "↑" : "↓"}
                        {Math.abs(d.progression).toFixed(1)}
                      </span>
                    )}
                  </div>
                ))}
      ...
      {showRating && (
        ...
        >
          <div
            className="bg-card rounded-2xl p-6 w-[85vw] max-w-sm shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <button
              ...
              className="w-full bg-primary text-primary-foreground py-3 rounded-xl font-bold transition-colors"
            >
              حفظ التقييم
            </button>
          </div>
