...
function StatBox({ label, value, sub, color = GOLD }: { label: string; value: string | number; sub?: string; color?: string }) {
  return (
    <div className="rounded-xl p-4 text-center" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 15%)" }}>
...
export default function MemberReport() {
  ...
  const totalVolume = filtered.reduce((sum: number, l: any) => sum + (parseFloat(l.weight) || 0) * (l.reps || 0), 0);
  ...
  const thisWeekVol = thisWeekLogs.reduce((s: number, l: any) => s + (parseFloat(l.weight) || 0) * (l.reps || 0), 0);
  const prevWeekVol = prevWeekLogs.reduce((s: number, l: any) => s + (parseFloat(l.weight) || 0) * (l.reps || 0), 0);
  ...
  return (
    <div className="p-4 space-y-4 pb-8">
        ...
        {periods.map(p => (
          <button key={p.key} onClick={() => setPeriod(p.key)}
            className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              period === p.key ? "text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"
            }`}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatBox label="إجمالي الحجم" value={totalVolume > 1000 ? `${(totalVolume / 1000).toFixed(1)}K` : totalVolume.toLocaleString()} sub="كجم × تكرار" />
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 15%)" }}>
        ...
        {byExercise.length === 0 ? (
          <div className="p-8 text-center"><p className="text-muted-foreground text-sm">لا يوجد سجل في هذه الفترة</p></div>
        ) : (
              ...
              return (
                <div key={ex.name} className="px-4 py-3 flex items-center justify-between">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{ex.logs.length} مجموعة · أقصى: {ex.maxWeight} كجم</p>
                  </div>
                  <div className="text-left flex-shrink-0">
                    <p className="text-sm font-bold" style={{ color: GOLD }}>{lastW} كجم</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-px" style={{ background: "hsl(0 0% 13%)" }}>
            {byExercise.slice(0, 6).map(ex => (
              <div key={ex.name} className="px-3 py-3 text-center" style={{ background: "hsl(0 0% 9%)" }}>
                <p className="text-lg font-black" style={{ color: GOLD }}>{ex.maxWeight}</p>
