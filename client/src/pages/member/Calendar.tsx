...
import { useListCheckins, useListExerciseLogs, getListCheckinsQueryKey, getListExerciseLogsQueryKey } from "@workspace/api-client-react";
...
export default function WorkoutCalendar() {
  ...
  const { data: logs } = useListExerciseLogs({ userId }, { query: { queryKey: getListExerciseLogsQueryKey({ userId }), enabled: !!userId } });

  const checkinList: any[] = Array.isArray(checkins) ? checkins : (checkins as any)?.data ?? [];
  const logList: any[] = Array.isArray(logs) ? logs : [];

  const checkinDates = new Set(checkinList.map((c: any) => {
    ...
    <div className="p-4 space-y-4 pb-8">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl p-3 text-center" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 14%)" }}>
          <p className="text-lg font-black" style={{ color: "hsl(142 60% 55%)" }}>{monthCheckins}</p>
          <p className="text-[10px] text-muted-foreground">يوم حضور</p>
        </div>
        <div className="rounded-xl p-3 text-center" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 14%)" }}>
          <p className="text-lg font-black" style={{ color: GOLD }}>{monthLogs}</p>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 14%)" }}>
        <div className="grid grid-cols-7 gap-0">
          {DAYS_AR.map(d => (
            <div key={d} className="text-center py-2 text-xs font-bold text-muted-foreground" style={{ borderBottom: "1px solid hsl(0 0% 14%)" }}>{d}</div>
          ))}
          {cells.map((day, i) => {
            ...
            return (
              <div key={day} className="py-2.5 text-center relative">
                <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold transition-all ${
                  ...
                  background: hasCheckin && hasLog ? `${GOLD}25` : hasCheckin ? "hsl(142 60% 55% / 0.15)" : hasLog ? "hsl(220 70% 65% / 0.15)" : "transparent",
                  color: hasCheckin || hasLog ? (hasCheckin && hasLog ? GOLD : hasCheckin ? "hsl(142 60% 60%)" : "hsl(220 70% 70%)") : "hsl(0 0% 50%)",
                }}>
                  {day}
                </span>
                {(hasCheckin || hasLog) && (
                  <div className="flex justify-center gap-0.5 mt-0.5">
                    {hasCheckin && <div className="w-1 h-1 rounded-full" style={{ background: "hsl(142 60% 55%)" }} />}
      ...
      <div className="flex items-center justify-center gap-4 text-xs text-muted-foreground">
        <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full" style={{ background: "hsl(142 60% 55%)" }} /> حضور</div>
        <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full" style={{ background: GOLD }} /> تمرين</div>
        <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full" style={{ background: `${GOLD}60` }} /> حضور + تمرين</div>
      </div>
    </div>
  );
}
