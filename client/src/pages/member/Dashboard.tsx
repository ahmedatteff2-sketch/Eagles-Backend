...
export default function MemberDashboard() {
  ...
  useEffect(() => {
    ...
    customFetch<any>(`/api/water?date=${today}`)
      .then((d) => setWater(d?.glasses ?? 0))
      .catch(() => {});
    customFetch<any[]>("/api/notifications")
      .then((d) => {
        const arr = Array.isArray(d) ? d : [];
        setNotifCount(arr.filter((n: any) => !n.read).length);
      })
      .catch(() => {});
  }, []);
  ...
  const totalDays =
    subData?.startDate && subData?.endDate
      ? Math.max(
          1,
          Math.ceil((new Date(subData.endDate).getTime() - new Date(subData.startDate).getTime()) / 86400000),
        )
      : 1;
  ...
  const thisWeek = checkinList.filter(
    (c: any) => new Date(c.timestamp ?? c.date ?? "").getTime() >= weekAgo.getTime(),
  ).length;
    ...
    <div className="p-4 sm:p-6 space-y-4 pb-8">
      ...
      >
        ...
        <div className="flex items-center gap-4 relative z-10">
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${GOLD}, ${G2})`,
              color: "hsl(0 0% 5%)",
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <div
          className="rounded-xl p-2.5 text-center"
          style={{
            background: streak >= 3 ? "hsl(25 90% 50% / 0.08)" : "hsl(0 0% 9%)",
            border: `1px solid ${streak >= 3 ? "hsl(25 90% 50% / 0.2)" : "hsl(0 0% 14%)"}`,
          }}
        >
          <p className="text-lg font-black" style={{ color: "hsl(25 90% 55%)" }}>
            {streak > 0 ? "🔥" : ""}
            {streak}
          </p>
        ...
        <div
          className="rounded-xl p-2.5 text-center"
          style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(200 80% 50% / 0.15)" }}
        >
          <p className="text-lg font-black" style={{ color: "hsl(200 80% 60%)" }}>
            {water}/{WATER_GOAL}
          </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div
          ...
          style={{
            ...
            border: `1px solid ${isActive ? "hsl(142 60% 50% / 0.25)" : "hsl(0 60% 50% / 0.25)"}`,
          }}
        >
          {isLoading ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-sm font-bold text-foreground truncate">
                    {subData.subscription?.name ?? "—"}
                  </p>
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-bold flex-shrink-0 ${isActive ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"}`}
                  >
                    {isActive ? "نشط" : "منتهي"}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  ...
                  ←{" "}
                  {new Date(subData.endDate).toLocaleDateString("ar-EG", { month: "short", day: "numeric" })}
                </p>
                {daysLeft !== null && isActive && (
                  <p
                    className="text-xs font-medium mt-1"
                    style={{ color: daysLeft <= 3 ? "hsl(0 60% 60%)" : "hsl(142 60% 55%)" }}
                  >
        ...
        >
          <div className="flex items-center gap-4">
            <div className="relative flex-shrink-0">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span
                  ...
                  style={{ color: todayPercent >= 100 ? "hsl(142 60% 55%)" : GOLD }}
                >
                  {todayPercent}%
                </span>
              </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {todayPercent >= 100 && (
                <p className="text-xs font-bold text-green-400 mt-1">أحسنت! أتممت الجلسة ✓</p>
              )}
              {todayPercent > 0 && todayPercent < 100 && (
                <Link href="/member/log">
                  <button
                    className="text-xs font-bold mt-1.5 px-3 py-1 rounded-lg"
              ...
              return (
                ...
                >
                  ...
                  <p className="text-base font-black" style={{ color: GOLD }}>
                    {lastW} <span className="text-xs text-muted-foreground font-normal">كجم</span>
                  </p>
                </div>
              );
            })}
      ...
      >
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className="text-xs"
            style={{ color: water >= WATER_GOAL ? "hsl(142 60% 55%)" : "hsl(200 80% 60%)" }}
          >
            {water >= WATER_GOAL ? "✓ أتممت الهدف!" : `${WATER_GOAL - water} أكواب متبقية`}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex-1 h-3 rounded-full overflow-hidden" style={{ background: "hsl(0 0% 14%)" }}>
            <div
              ...
              style={{
                ...
                background: water >= WATER_GOAL ? "hsl(142 60% 50%)" : "hsl(200 80% 55%)",
              }}
            />
          </div>
          <div className="flex gap-1">
            <button
              onClick={() => addWater(-1)}
              className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold transition-colors bg-muted text-muted-foreground hover:bg-muted/80"
            >
              −
            </button>
            <button
              ...
              style={{ background: "hsl(200 80% 50% / 0.2)", color: "hsl(200 80% 60%)" }}
            >
              +
            </button>
          </div>
        ...
        <div className="flex gap-0.5 mt-2">
          {Array.from({ length: WATER_GOAL }).map((_, i) => (
            <div
              ...
              style={{ background: i < water ? "hsl(200 80% 55%)" : "hsl(0 0% 14%)" }}
            />
          ))}
        </div>
      </div>
        ...
        {quickLinks.map((item) => (
          <Link key={item.href} href={item.href}>
            <div
              ...
              onMouseLeave={(e) => {
                (e.currentTarget as HTMLElement).style.borderColor = "hsl(0 0% 14%)";
              }}
            >
      ...
      <Link href="/member/log">
        <div
          className="md:hidden fixed bottom-20 left-4 z-30 w-14 h-14 rounded-full flex items-center justify-center shadow-xl cursor-pointer active:scale-95 transition-transform"
          style={{
            background: `linear-gradient(135deg, ${GOLD}, ${G2})`,
            boxShadow: `0 4px 20px hsl(40 65% 48% / 0.4)`,
          }}
        >
          <svg
            viewBox="0 0 24 24"
