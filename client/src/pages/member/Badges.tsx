  ...
  return (
    <div className="p-4 space-y-4 pb-8">
      <div>
        ...
        <p className="text-muted-foreground text-sm">{earned.length} من {badges.length} إنجاز محقق</p>
      </div>
      ...
      <div className="rounded-xl p-4" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 14%)" }}>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm font-black tabular-nums" style={{ color: GOLD }}>{Math.round((earned.length / badges.length) * 100)}%</span>
        </div>
        <div className="h-3 rounded-full overflow-hidden" style={{ background: "hsl(0 0% 14%)" }}>
          <div className="h-full rounded-full transition-all duration-700" style={{ width: `${(earned.length / badges.length) * 100}%`, background: GOLD }} />
        </div>
      </div>
      ...
      {earned.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {earned.map(b => (
              <div key={b.id} className="rounded-xl p-3 text-center" style={{ background: "hsl(40 65% 48% / 0.08)", border: "1px solid hsl(40 65% 48% / 0.2)" }}>
                <div className="text-3xl mb-1">{b.icon}</div>
                <p className="text-xs font-bold" style={{ color: GOLD }}>{b.name}</p>
      ...
      {locked.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {locked.map(b => (
              <div key={b.id} className="rounded-xl p-3 text-center opacity-50" style={{ background: "hsl(0 0% 9%)", border: "1px solid hsl(0 0% 14%)" }}>
