...
const bodyStatSchema = z.object({
  ...
  arm: z.number().min(0).max(200).nullable().optional(),
  thigh: z.number().min(0).max(200).nullable().optional(),
  calf: z.number().min(0).max(200).nullable().optional(),
  shoulders: z.number().min(0).max(300).nullable().optional(),
  neck: z.number().min(0).max(200).nullable().optional(),
...
router.post("/body-stats", authenticate, async (req, res) => {
  ...
  try {
    const [stat] = await db.insert(bodyStatsTable).values({
      ...
      chest: body.data.chest != null ? String(body.data.chest) : null,
      waist: body.data.waist != null ? String(body.data.waist) : null,
      hips: body.data.hips != null ? String(body.data.hips) : null,
      arm: body.data.arm != null ? String(body.data.arm) : null,
      thigh: body.data.thigh != null ? String(body.data.thigh) : null,
      calf: body.data.calf != null ? String(body.data.calf) : null,
      shoulders: body.data.shoulders != null ? String(body.data.shoulders) : null,
      neck: body.data.neck != null ? String(body.data.neck) : null,
