    ...
    createUser.mutate({ data: userPayload }, {
      onSuccess: (newUser: any) => {
        ...
        if (subscriptionId && startDate && userId) {
          assignSub.mutate({
            data: { userId, subscriptionId, startDate,
                      ...
                      <span
                        ...
                        style={{
                          background: u.currentSubscription.status === "active"
                            ? "hsl(142 60% 50% / 0.15)" : "hsl(0 0% 20%)",
                          color: u.currentSubscription.status === "active"
                            ? "hsl(142 60% 60%)" : "hsl(0 0% 50%)",
                        }}
                      >
        ...
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4" style={{ backdropFilter: "blur(4px)" }}>
          ...
          >
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid hsl(0 0% 14%)" }}>
              ...
              <button onClick={() => setShowCreate(false)} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground" style={{ background: "hsl(0 0% 14%)" }}>
            ...
            <form onSubmit={createForm.handleSubmit(onSubmitCreate)} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div className="text-xs font-bold uppercase tracking-widest pb-2 mt-2" style={{ color: "hsl(40 65% 48%)", borderBottom: "1px solid hsl(40 65% 48% / 0.2)" }}>الاشتراك (اختياري)</div>
              ...
              <FieldInput label="خطة الاشتراك">
                <select {...createForm.register("subscriptionId")} className={inputClass} style={inputStyle}>
                  ...
                  {subList.map((s: any) => (
                    <option key={s.id} value={s.id}>{s.name} — {s.duration} يوم — {s.price} ج.م</option>
                  ))}
                </select>
              </FieldInput>
                ...
                <>
                  {selectedPlan && (
                    <div className="rounded-lg px-3 py-2.5 text-xs flex items-center gap-2" style={{ background: "hsl(142 60% 50% / 0.08)", border: "1px solid hsl(142 60% 50% / 0.2)", color: "hsl(142 60% 60%)" }}>
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 flex-shrink-0"><polyline points="20 6 9 17 4 12" /></svg>
                      <span>سيتم تعيين اشتراك <strong>{selectedPlan.name}</strong> — {selectedPlan.duration} يوم بسعر <strong>{selectedPlan.price} ج.م</strong></span>
                    </div>
                  )}
                  <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={createUser.isPending || assignSub.isPending} className="flex-1 py-2.5 rounded-lg text-sm font-bold transition-all disabled:opacity-50" style={{ background: "linear-gradient(135deg, hsl(40 65% 52%), hsl(40 65% 42%))", color: "hsl(0 0% 5%)" }}>
                  {createUser.isPending || assignSub.isPending ? "جاري الحفظ..." : "إضافة العضو"}
                </button>
                <button type="button" onClick={() => setShowCreate(false)} className="flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all" style={{ background: "hsl(0 0% 14%)", color: "hsl(0 0% 70%)" }}>
                  إلغاء
                </button>
              </div>
            </form>
      ...
      {editingUser && (
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4" style={{ backdropFilter: "blur(4px)" }}>
          ...
          >
            ...
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid hsl(0 0% 14%)" }}>
              <div>
                <h2 className="text-base font-bold text-foreground">تعديل بيانات العضو</h2>
                <p className="text-xs text-muted-foreground mt-0.5">{editingUser.name}</p>
              </div>
              <button onClick={() => setEditingUser(null)} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground" style={{ background: "hsl(0 0% 14%)" }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
                  <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
                </svg>
              </button>
            ...
            <form onSubmit={editForm.handleSubmit(onSubmitEdit)} className="p-6 space-y-4">
              <FieldInput label="الاسم الكامل" error={editForm.formState.errors.name?.message}>
                <input {...editForm.register("name")} placeholder="أحمد محمد" className={inputClass} style={inputStyle} />
              </FieldInput>

              <FieldInput label="رقم الهاتف" error={editForm.formState.errors.phone?.message}>
                <input {...editForm.register("phone")} placeholder="01xxxxxxxxx" className={inputClass} style={inputStyle} />
              </FieldInput>
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={updateUser.isPending} className="flex-1 py-2.5 rounded-lg text-sm font-bold transition-all disabled:opacity-50" style={{ background: "linear-gradient(135deg, hsl(40 65% 52%), hsl(40 65% 42%))", color: "hsl(0 0% 5%)" }}>
                  {updateUser.isPending ? "جاري الحفظ..." : "حفظ التعديلات"}
                </button>
                <button type="button" onClick={() => setEditingUser(null)} className="flex-1 py-2.5 rounded-lg text-sm font-semibold" style={{ background: "hsl(0 0% 14%)", color: "hsl(0 0% 70%)" }}>
                  إلغاء
                </button>
              </div>

              {/* Reset password section */}
              <div className="pt-2 border-t" style={{ borderColor: "hsl(0 0% 16%)" }}>
                <button
                  type="button"
          ...
          >
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid hsl(0 0% 14%)" }}>
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{resettingUser.name}</p>
              </div>
            ...
            <form onSubmit={resetForm.handleSubmit(onSubmitReset)} className="p-6 space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button type="button" onClick={() => setResettingUser(null)} className="flex-1 py-2.5 rounded-lg text-sm font-semibold" style={{ background: "hsl(0 0% 14%)", color: "hsl(0 0% 70%)" }}>
                  إلغاء
                </button>
              </div>
            </form>
